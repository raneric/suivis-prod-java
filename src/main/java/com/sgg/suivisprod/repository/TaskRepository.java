...
public interface TaskRepository extends PagingAndSortingRepository<Task, String> {
	...
	@Query(value = "{'user.username':?0,'taskType':?1}")
	List<Task> findAllByUserNameAndTaskType(String username, String taskType);
	...
	@Aggregation(pipeline = { "{'$match':{'user.username':?0,'taskState':?1}}", "{'$count':'totalTask'}" })
	Integer countTaskByTaskState(String userName, String taskState);
